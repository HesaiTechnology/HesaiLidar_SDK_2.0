#ifndef HS_LIDAR_ET_V7_H
#define HS_LIDAR_ET_V7_H
...
namespace hesai 
{
  namespace lidar
  {
    ...
    // Body
    // body_unit
    struct HS_LIDAR_BODY_LASTER_UNIT_ET_V7 {
      uint16_t distance;
    ...
    struct HS_LIDAR_BODY_SEQ3_ET_V7 {
    ...
    // Body
    // crc
    struct HS_LIDAR_BODY_CRC_ET_V7 {
      uint32_t m_u32Crc;

      uint32_t GetCrc() const { return little_to_native(m_u32Crc); }

      void Print() const {
        printf("HS_LIDAR_BODY_CRC_ET_V7:\n");
        printf("crc:0x%08x\n", GetCrc());
      }
    } PACKED;
  ...
  long int timezone =  tzi.Bias * 60;
    ...
    // Tail
    // seqnum
    struct HS_LIDAR_TAIL_SEQ_NUM_ET_V7 {
      ...
      void Print() const {
        printf("HS_LIDAR_TAIL_SEQ_NUM_ET_V7:\n");
        printf("seqNum: %u\n", GetSeqNum());
      }
    } PACKED;
    ...
    // Tail
    // crc
    struct HS_LIDAR_TAIL_CRC_ET_V7 {
      uint32_t m_u32Crc;
      uint32_t GetCrc() const { return little_to_native(m_u32Crc); }
      void Print() const {
        printf("HS_LIDAR_TAIL_CRC_ET_V7:\n");
        printf("crc:0x%08x\n", GetCrc());
      }
    } PACKED;

    // Cyber Security (optional)
    struct HS_LIDAR_CYBER_SECURITY_ET_V7 {
      ...
      void Print() const {
        printf("HS_LIDAR_CYBER_SECURITY_ET_V7:\n");
    ...
    struct HS_LIDAR_HEADER_ET_V7 {
      ...
      uint16_t GetPacketSize() const {
        return (sizeof(struct HS_LIDAR_PRE_HEADER) + sizeof(struct HS_LIDAR_HEADER_ET_V7) + 
                (sizeof(uint16_t) + (sizeof(struct HS_LIDAR_BODY_LASTER_UNIT_ET_V7) * (m_u8LaserNum / m_u8SeqNum) + 5) * m_u8SeqNum) * m_u8BlockNum +
                sizeof(struct HS_LIDAR_BODY_CRC_ET_V7) + 
                sizeof(struct HS_LIDAR_TAIL_ET_V7) + sizeof(struct HS_LIDAR_TAIL_SEQ_NUM_ET_V7) + sizeof(struct HS_LIDAR_TAIL_CRC_ET_V7) + 
                sizeof(struct HS_LIDAR_CYBER_SECURITY_ET_V7)
               );
      }
      void Print() const {
        printf("HS_LIDAR_HEADER_ET_V7:\n");
        printf(
        "laserNum:%02u, block_num:%02u, DistUnit:%g, "
        "EchoNum:%02u, HasSeqNum:%d, HasIMU:%d, "
        "HasFuncSafety:%d, HasCyberSecurity:%d, HasConfidence:%d\n",
        GetLaserNum(), GetBlockNum(), GetDistUnit(),
